...
def predict(model, scale, data, is_force=True):
	"""
	data from fp_cal
	"""
	# preprocess 
	...
	n_clusters, n_atoms, n_fp = b_fp.size(0), b_fp.size(1), b_fp.size(2)
	b_dfpdX = data['b_dfpdX'].reshape(n_clusters, n_atoms*n_fp, n_atoms*3)
	nrg_pre_raw = model(b_fp)  # [n_cluster, n_atoms, n_elements]
	nrg_pre_atom = torch.sum(nrg_pre_raw * data['b_e_mask'], dim=2)
...
def get_scaling(train_data, add_const=1e-10):
	...
	add_const: constant added into demominator when scaling the energy and force
		       otherwise, it is possible to get super large force (e.g, when nrg_min is closed to nrg_max)
	"""
	...
	scale = {'fp_max': fp_max[0], 'fp_min': fp_min[0], 'nrg_max': nrg_max, 'nrg_min': nrg_min, 'add_const': add_const}
	return scale


def scale_data(data, scale):
	scale_const = scale['add_const']
	data['b_fp'] = (data['b_fp'] - scale['fp_min']) / (scale['fp_max'] - scale['fp_min'] + scale_const)
	# data['b_e'] = (data['b_e'] - scale['nrg_min']) / (scale['nrg_max'] - scale['nrg_min'] + scale_const)
	# data['b_f'] = data['b_f'] / (scale['nrg_max'] - scale['nrg_min'] + scale_const)
	data['b_dfpdX'] /= (scale['fp_max'] - scale['fp_min'] + scale_const).view(1, 1, -1, 1, 1)
	return data
...
class Agent(object):
	def __init__(self, train_data, valid_data, scale_const, model_path, test_data=None, layer_nodes=[10, 10], 
				 activation=['tanh', 'tanh'], lr=1, max_iter=20, history_size=100, device=torch.device('cpu')):
		"""
	...
	def train(self, log_name, n_epoch=1000, interupt=True, val_interval=10, is_force=True, nrg_convg=1, force_convg=5, max_frs_convg=25, nrg_coef=1, force_coef=25):
		...
		train_actual_atoms = self.train_data['N_atoms']
		...
		valid_actual_atoms = self.valid_data['N_atoms']
		...
		if self.test_data is not None:
			...
			test_actual_atoms = self.test_data['N_atoms']
		...
		if self.test_data is not None:				
			...
			test_nrg_mae = mae(test_nrg_pre_cluster / test_actual_atoms, test_nrg_label_cluster / test_actual_atoms) * self.scale_const
			...
			if is_force:
				...
				test_force_mae = sum_l1(test_force_pre, test_force_label) / test_actual_atoms.sum() / 3 * self.scale_const
		...
		for epo in range(n_epoch):
			...
			for i_b in range(n_batchs):
				...
				def closure():
					...
					train_nrg_pre_raw = self.model(batch_fp)
					...
					train_loss = mse(train_nrg_pre_cluster/train_actual_atoms[s_idx], 
									train_nrg_label_cluster[s_idx]/train_actual_atoms[s_idx]) * nrg_coef # * (self.scale_const)**2
					...
					if is_force:
						...
						train_force_loss = mse(train_force_pre, train_force_label[s_idx]) * force_coef # * (self.scale_const)**2
			...
			if epo % val_interval == 0:
				...
				valid_nrg_mae = mae(valid_nrg_pre_cluster/valid_actual_atoms, valid_nrg_label_cluster/valid_actual_atoms) * self.scale_const
				if is_force:
					...
					valid_force_mae = sum_l1(valid_force_pre, valid_force_label) / valid_actual_atoms.sum() / 3 * self.scale_const
					valid_force_max = torch.abs(valid_force_pre - valid_force_label).max() * self.scale_const
				else:
					valid_force_mae = 0
					valid_force_max = 0
				
				with open(log_name, 'a') as file:
					file.write(f'validation: epoch: {epo}, nrg_mae: {valid_nrg_mae*1000 : .3f} meV/atom,' + 
					           f'force_mae: {valid_force_mae*1000 : .3f} meV/AA, max fae: {valid_force_max*1000 : .3f} meV/AA \r\n')
				# print(f'validation: epoch: {epo}, n_mae: {valid_nrg_mae*1000 : .3f} meV/atom, ' +
				# 	  f'f_mae: {valid_force_mae*1000 : .3f} meV/AA, max fe: {valid_force_max*1000 : .3f} meV/AA \r\n')
				...
				self.save_model()
				if interupt and (valid_nrg_mae*1000 <= nrg_convg) and \
					 (valid_force_max*1000 <= max_frs_convg) and (valid_force_mae*1000 <= force_convg):
	...
	def test(self, is_force):
		...
		test_actual_atoms = test_data['N_atoms']
		if is_force:
			test_b_fp.requires_grad = True
		...
		if is_force:
			...
			force_mae = sum_l1(force_pre, test_force_label) / test_actual_atoms.sum() / 3
		else:
			force_pre = 0
			force_mae = 0
